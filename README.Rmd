...
* __Visualizing posteriors__. The focus on tidy data makes the output from tidybayes
  ...
  densities with point summaries and intervals ("eye plots", `stat_eye` / `stat_eyeh`;
  or "half-eye plots", `stat_halfeye` / `stat_halfeyeh`), and visualizing fit lines with an arbitrary 
  number of uncertainty bands (`geom_lineribbon` and `stat_lineribbon`). Combining the
  base-R `quantile` function with `geom_dotplot` also facilitates the construction
