...
#' use \code{\link{geom_pointinterval}} and \code{\link{geom_pointintervalh}}
...
#' function, these \code{stat}s are intended for use directly on samples, and
...
#' \code{geom_pointinterval}/\code{geom_pointinterval} and \code{stat_pointinterval}/\code{stat_pointintervalh}.
...
#'   return a data frame with variables \code{x}, \code{ymin} and \code{ymax}
#'   (\code{xmin} and \code{xmax} for \code{stat_pointintervalh}),
