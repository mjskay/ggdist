...
#' with output from \code{\link{point_interval}}. This geom sets some default aesthetics equal to the \code{conf.low},
#' \code{conf.high}, and \code{.prob} columns generated by the \code{point_interval} family of functions, making them
...
#' \code{aes(ymin = conf.low, ymax = conf.high, size = -.prob)}.
#'
...
geom_lineribbon <- function(mapping = NULL, data = NULL,
  ...
  default_computed_aesthetics = aes(ymin = conf.low, ymax = conf.high, fill = forcats::fct_rev(ordered(.prob)))
