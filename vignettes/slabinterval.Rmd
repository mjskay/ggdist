...
dists_plot = dists_df %>%
  ...
  scale_slab_alpha_continuous(range = c(0,1), guide = FALSE) +
...
Adding `scale_slab_alpha_continuous(range = c(0,1))` above is helpful as without it the gradients have a hard boundary, which may not be desirable:

```{r gradient_dodge_bad_alpha}
df %>%
  ggplot(aes(x = group, y = value, fill = subgroup)) +
  stat_gradientinterval(position = "dodge") +
  labs(
...
dist_df %>%
  ...
  scale_slab_alpha_continuous(range = c(0,1)) +
  labs(
    title = "stat_dist_gradientinterval(position = 'dodge')",
    alpha = "density"
  )
```
