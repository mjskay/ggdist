...
go into a density estimate. To use multiple slab geometries together, you can use the
`side` parameter to change which side of the interval a slab is drawn on and set the
`scale` parameter to something around `0.5` (by default it is `0.9`) so that the
two slabs do not overlap:

```{r halfeye_dotplot, fig.width = med_width, fig.height = small_height}
df %>%
  ggplot(aes(x = group, y = value)) +
...
In this example, depending on your use case you might prefer a quantile dotplot
instead of a dotplot of all data points:
