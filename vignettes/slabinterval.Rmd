...
dists_plot = dists_df %>%
  ...
  stat_interval(aes(x = x, y = "interval", dist = NULL, args = NULL), data = hist_df, color = "gray65", alpha = 1/3, size = 10,
    position = position_nudge(y = -.1)) +
  stat_pointinterval(aes(x = x, y = "pointinterval", dist = NULL, args = NULL), data = hist_df) +
  ...
  stat_histinterval(aes(x = x, dist = NULL, args = NULL), data = hist_df, position = position_nudge(y = - 0.4)) +
...
**Note on "choppy" gradients:** Depending on your graphics device, gradients may be "choppy" looking. 
...
function (such as `pdf()`, `svg()`, or `png(type = "cairo")`; see [here](https://developer.r-project.org/Blog/public/2020/07/15/new-features-in-the-r-graphics-engine/)):
...
For example, replacing `stat_halfeye()` with `stat_dots()` produces dotplots. With so few dots here, the outlines mask the fill, so it makes sense to map the outline color of the dots as well:
...
Unlike the base `ggplot2::geom_dotplot()` geom, `ggdist::geom_dots()` automatically determines a bin width to ensure that the dot stacks fit within the available space. 
...
`"weave"` layout, which positions dots at (or very close to) their true positions,
...
which tends to work well on sample data (but not on quantiles), and especially when
`side = "both"`:
...
Thus there are three layout options with `dots` and `dotsinterval` stats and
geoms:
...
the `cut_cdf_qi()` function, which labels points in a CDF according to
...
We could similarly use the `stat_dist_interval()` (or `stat_interval()`) geometries
...
the `stat_dist_...` family which contains the level of the generated intervals,
...
a geom, allowing (for example) different groups to hang above or below the interval:
...
The `stat_dist_...` family will automatically detect discrete distributions and
...
using `outline_bars = TRUE` or `FALSE` (the default is `FALSE`). Here is an example
of a redundant encoding of thickness and fill color on stepped histograms inspired
by an example from Isabella Ghement:
...
go into a density estimate, creating so-called "rain cloud" plots. To use multiple
...
When constructing composite plots it may be useful to position slab and interval parts of the geometry separately.
