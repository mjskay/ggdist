...
The scaling of slab `thickness` is determined by a combination of the geometry's `height`/`width`, its `scale`, the `normalize` parameter, and any `thickness` scales added to the plot (such as `scale_thickness_shared()`). For complete details on the `thickness` aesthetic and how to adjust slab scaling, see the [thickness article](https://mjskay.github.io/ggdist/articles/thickness.html).
...
**Note**: subguides are currently only available in the development version
of ggdist (> 3.3.1) on Github.
...
their curves. 
Futher details of scaling of the `thickness` aesthetic are discussed in the
