...
```{r slabinterval_family, fig.height = 5, fig.width = 6.5, echo = FALSE}
dists_df = tibble(
  # enforce order
  geom = rev(c(
    "halfeye[h]", 
...
label_annotations = tribble(
  ~x, ~y, ~label,
  2, 1, "stat_... only",
  2, 2.5, "stat_... and geom_...",
  2, 6, "stat_... and stat_dist_...",
)

annotation_plot = line_annotations %>%
  ggplot(aes(x, y)) +
  geom_segment(aes(xend = x2, yend = y2), lineend = "square", color = "gray70") +
  scale_y_continuous(limits = c(0.4, nlevels(dists_df$geom) + 0.6), breaks = NULL) +
  scale_x_continuous(limits = c(0, 20), breaks = NULL) +
  geom_text(aes(label = label), data = label_annotations, hjust = 0, vjust = 0.3, size = 3.5, color = "gray30") +
  coord_cartesian(expand = FALSE, clip = "off") +
  labs(x = NULL, y = NULL)

plot_grid(ncol = 2, align = "h", rel_widths = c(0.7, 0.3),
  dists_plot,
  annotation_plot
)
```
...
```{r eye_side, fig.width = 7, fig.height = 4}
p = df %>%
  ggplot(aes(x = group, y = value)) +
  panel_border()
...
```{r halfeyeh_family, fig.width = 6, fig.height = 6}
p = df %>%
  ggplot(aes(x = group, y = value)) +
  panel_border()
...
```{r eyeh_side, fig.width = 7, fig.height = 3}
p = df %>%
  ggplot(aes(x = value, y = group)) +
  panel_border()
...
Unlike with geom_violin(), densities in groups that are not dodged (here, 'a' and 'b')\nhave the same area and max width as those in groups that are dodged ('c'):
...
```{r stat_histintervalh, fig.width = 6, fig.height = 3}
p = df %>%
  ggplot(aes(x = group, y = value)) +
  panel_border()
...
```{r cdfinterval_family, fig.width = 6, fig.height = 6}
p = df %>%
  ggplot(aes(x = group, y = value)) +
  panel_border()
...
```{r ccdf_side, fig.height = 2.5, fig.width = 7}
p = df %>%
  ggplot(aes(x = value, y = group)) +
  expand_limits(x = 0) +
  panel_border()
...
Finally, an alternative approach to mapping density onto the `thickness` aesthetic of the slab is to instead map it onto its `alpha` value (i.e., opacity). This is what the `stat_[dist_]gradientinterval[h]` family does (actually, it uses `slab_alpha`, a variant of the `alpha` aesthetic).
