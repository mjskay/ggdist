...
* Stats starting with `stat_` are meant to be used on sample data; e.g. draws from a posterior distribution (or any other distribution, really). These stats compute relevant summaries (densities, CDFs, points, and/or intervals) before forwarding the summaries to their geom. Many have geom counterparts (e.g. `stat_interval()` corresponds to `geom_interval()`, except the former applies to sample data and the latter to already-summarized data). Many of these stats do not currently have geom counterparts (e.g. `stat_ccdfbar`), as they are not really that useful without the summarization step.
...
For all of the examples above, both on sample data and analytical distributions, you can replace `[half]eye` with `[c]cdfbar` to get a stat that creates a CDF or CCDF bar plot. 

### On sample data: `stat_[c]cdfbar[h]`

For example, replacing `stat_eye()` with `stat_ccdfbar()` produces CCDF bar plots:

```{r}
df %>%
  ggplot(aes(x = group, y = value, fill = subgroup)) +
  stat_ccdfbar(position = "dodge") +
  ggtitle(
    "stat_ccdfbar(position = 'dodge')",
...
df %>%
  ggplot(aes(x = group, y = value, fill = subgroup)) +
  stat_ccdfbar(position = "dodge", justification = 1) +
  expand_limits(y = 0) +
  ggtitle(
    "stat_ccdfbar(position = 'dodge', justification = 1)",
    subtitle = "aes(x = group, y = value, fill = subgroup)"
...
The `side` parameter also works as with `stat_eye()`. Here we'll demonstrate with `stat_ccdfbarh()`, the horizontal version:
...
plot_grid(ncol = 3, align = "hv", 
  # side = "left" would give the same result
  p + stat_ccdfbarh(side = "bottom") + ggtitle("stat_ccdfbarh()") + labs(subtitle = "side = 'bottom'"),
  p + stat_ccdfbarh(side = "both") + labs(subtitle = "side = 'both'"),
  # side = "right" would give the same result
  p + stat_ccdfbarh(side = "top") + labs(subtitle = "side = 'top'")
)
```

### On analytical distributions: `stat_dist_[c]cdfbar[h]`

You can also use `stat_dist_ccdfbar()` instead if you wish to visualize analytical distributions, just as you can use `stat_dist_eye()`.

By default, `stat_dist_ccdfbar()` uses the quantiles at `p = 0.001` and `p = 0.999` in the distributions are used to determine their extrant. You can change this setting using the `p_limits` parameter, or use `expand_limits()` to ensure a particular value is shown as before:

```{r}
dist_df %>%
  ggplot(aes(x = group, dist = "norm", arg1 = mean, arg2 = sd, fill = subgroup)) +
  stat_dist_ccdfbar(position = "dodge") +
  expand_limits(y = 0) +
  ggtitle(
    "stat_dist_ccdfbar(position = 'dodge')",
    subtitle = "aes(dist = 'norm', arg1 = mean, arg2 = sd)"
...
### Summing up CDF bar plots: `stat_[dist_][c]cdfbar[h]`

All of the above geoms follow the naming scheme `stat_[dist_][c]cdfbar[h]`.
