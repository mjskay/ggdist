...
`stat_dist_slabh()` is a shortcut geom for something like `stat_dist_halfeyeh(show_interval = FALSE, ...)`, with some simplifications: since it does not have the interval attached to it, it does not require you to use `slab_color` to target the outline color; you can use `color` directly. This simplifies things somewhat:
