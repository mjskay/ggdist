...
abc_df %>%
  ggplot(aes(x = abc, y = value)) +
  geom_dots(side = "both", layout = "hex") +
...
abcc_df %>%
  ggplot(aes(y = value, x = abc, fill = hi, linewidth = hi)) +
...
ggplot() +
  geom_dots(aes(x = x_large)) +
  labs(
    title = "geom_dots(aes(x = x_large))",
    subtitle = "where length(x_large) = 2000",
    x = NULL
  ) +
  scale_y_continuous(breaks = NULL)
```

The `smooth_dots()` function can be applied directly to the variable to smooth it:

```{r large_sample_smooth, fig.width = small_width, fig.height = small_width/2}
ggplot() +
  geom_dots(aes(x = smooth_dots(x_large))) +
  labs(
    title = "geom_dots(aes(x = smooth_dots(x_large)))",
    subtitle = "where length(x_large) = 2000",
    x = NULL
  ) +
  scale_y_continuous(breaks = NULL)
...
This works particularly well with the `"hex"` layout:

```{r large_sample_smooth_hex, fig.width = small_width, fig.height = small_width/2}
ggplot() +
  geom_dots(aes(x = smooth_dots(x_large)), layout = "hex", stackratio = 0.9) +
  labs(
    title = 'geom_dots(aes(x = x_large), layout = "hex", stackratio = 0.9)',
    subtitle = "where length(x_large) = 2000",
    x = NULL
  ) +
  scale_y_continuous(breaks = NULL)
