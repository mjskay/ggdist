...
Like with `b[term,group]`, this gives us a tidy data frame. If we want the median and 95% quantile interval of the parameters, we can apply `median_qi`:
...
We can specify the columns we want to get medians and intervals from, as above, or if we omit the list of columns, `median_qi` will use every column that is not a grouping column or a special column (one that starts with `.`, like `.chain` or `.iteration`). Thus in the above example, `(Intercept)` and `sigma` are redundant arguments to `median_qi` because they are also the only columns we gathered from the model. So we can simplify this to:
...
When we have a parameter with one or more indices, such as `b`, we can apply `median_qi` (or other functions in the `point_estimate` family) as we did before:
...
How did `median_qi` know what to aggregate? Data frames returned by `spread_samples` are automatically grouped by all index variables you pass to it; in this case, that means `spread_samples` groups its results by `group`. `median_qi` respects those groups, and calculates the estimates and intervals within all groups. Then, because no columns were passed to `median_qi`, it acts on the only non-special (`.`-prefixed) and non-group column, `b`. So the above shortened syntax is equivalent to this more verbose call:
...
m %>%
  spread_samples(`(Intercept)`, b[,group]) %>%
  median_qi(condition_mean = `(Intercept)` + b, .prob = c(.95, .8, .5))
```
...
m %>%
  spread_samples(`(Intercept)`, b[,group]) %>%
  median_qi(condition_mean = `(Intercept)` + b, .prob = c(.95, .66)) %>%
