...
In this particular model, there is only one term (`(Intercept)`), thus we could omit that index altogether to just get each `condition` and the value of `b` for that condition:
