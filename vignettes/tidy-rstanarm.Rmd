...
We will prefer the more descriptive and less cryptic names.
...
How did `mean_qi` know what to aggregate? Data frames returned by `gather_samples` are automatically grouped by all index variables you pass to it; in this case, that means `gather_samples` groups its results by `term`, `group`, and `condition`. `mean_qi` respects those groups, and calculates the estimates and intervals within all groups. Then, because no columns were passed to `mean_qi`, it acts on the only non-special (`.`-prefixed) and non-group column, `b`. So the above shortened syntax is equivalent to this more verbose call:

```{r}
m %>%
    gather_samples(b[term,group,condition]) %>%
    group_by(term,group,condition) %>%    # this line not necessary (done by gather_samples)
    mean_qi(b)
```
...
```{r}
m %>% 
    gather_samples(`(Intercept)`, b[term,group,condition])
```
...
```{r}
m %>%
    gather_samples(`(Intercept)`, b[term,group,condition], sigma) %>%
