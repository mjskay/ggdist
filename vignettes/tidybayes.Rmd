...
`gather_samples` is the counterpart to `spread_samples`, except it puts all parameter names into a `terms` column and samples into an `estimate` column (for compatibility with `broom::tidy`):
