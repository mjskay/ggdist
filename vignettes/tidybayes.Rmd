...
### Using `geom_pointrange`/`geom_pointrangeh`

Plotting means and intervals is straightforward using the "pointrange" geom of `ggplot` (or its horizontal version, "pointrangeh" from `ggstance`):
...
### Using `stat_summary`/`stat_summaryh`

Rather than summarizing the posterior before calling ggplot, we could also use `stat_summary` to perform the summary within ggplot:

```{r}
m %>%
  spread_samples(condition_mean[condition]) %>%
  ggplot(aes(y = fct_rev(condition), x = condition_mean)) +
  stat_summaryh(fun.data = mean_qih)
```
...
If you prefer densities over violins, you can use `geom_halfeyeh` (the vertical version, not yet available, will be called `geom_helfeye`). This example also demonstrates how to change the interval probability (here, to 80% intervals):

```{r}
m %>%
  spread_samples(condition_mean[condition]) %>%
  ggplot(aes(y = fct_rev(condition), x = condition_mean)) +
  geom_halfeyeh(fun.args = list(.prob = .8))
```
...
m %>%
  ...
  geom_pointrangeh(fatten = 2) +    # smaller point estimate (otherwise it is very large)
...
## Comparison with models using `broom::tidy`

Because mean_qi and related functions use a similar naming scheme to that of `broom::tidy`, it is easy to compare results against models support by `broom::tidy`. For example, let's compare our model estimates of condtional means against an ordinary least squares (OLS) regression:

```{r}
m_linear = lm(response ~ condition, data = ABC)
```

Combining `lsmeans` with `broom::tidy`, we can generate tidy-format estimates of parameters from the above model:
...
And plot them together to compare:
...
## Long format parameter data frames: `gather_samples` and `gather_terms`
...
parameters = samples %>%
  mean_qi(condition_mean, .prob = c(.95, .50))

ABC %>%
  ggplot(aes(y = condition)) +
  geom_linerangeh(aes(xmin = conf.low, xmax = conf.high, 
    color = ordered(-.prob)), size = 4, data = reps) +
  geom_pointrangeh(aes(x = condition_mean, xmin = conf.low, xmax = conf.high, 
    size = -.prob), fatten = 1.5, position = position_nudge(y = -0.2),
    data = parameters) +
...
The above code block also shows the use of `mean_qih`. The `[point]_[interval]h` family of functions act like the `[point]_[interval]` functions, except they return a data frame with `x`/`xmin`/`xmax` instead of `y`/`ymin`/`ymax` when given a vector, making them suitable for use with `stat_summaryh` (whereas `[point]_[interval]` can be used with `stat_summary`).


## Interoperability with `bayesplot` and other packages
...
It may be desirable to use the `spread_samples` or `gather_samples` functions to transform your samples in some way, and then convert them *back* into the sample $\times$ parameter format to pass them into functions from `bayesplot` or other packages like it. The `unspread_samples` and `ungather_samples` functions invert `spread_samples` and `gather_samples` to return a data frame with parameter names that including indices in them as columns and samples as rows.
...
m %>%
  spread_samples(condition_mean[condition]) %>%
  ggplot(aes(y = fct_rev(condition), x = condition_mean)) +
  geom_eyeh(fun.data = mean_qih) + 
  stat_summaryh(fun.data = mode_hdih, position = position_nudge(y = .15), color="red")
```

The difference is probably more noticeable given a skewed distribution, such as might be expected on a scale parameter:
...
m %>%
  ...
  stat_summaryh(aes(y = "mean_qi"), fun.data = mean_qih) +
  stat_summaryh(aes(y = "mode_hdi"), fun.data = mode_hdih) +
  ylab("")
```
