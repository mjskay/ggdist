...
This gives us a tidy data frame, with one column per parameter and one sample per row. If we want the mean and 95% quantile interval of the parameters, we can use `mean_qi`:
...
We can specify the columns we want to get means and intervals from, as above, or if we omit the list of columns, `mean_qi` will use every column that is not a grouping column or a special column (one that starts with `.`, like `.chain` or `.iteration`). Thus in the above example, `overall_mean` and `y_sd` are redundant arguments to `mean_qi` because they are also the only columns `mean_qi` would aggregate when passed an empty list:
...
What's more interesting, though, is when we have a parameter with one or more indices, such as `group_mean`. We can extract this variable using gather_samples as well, by giving a variable name to become the index column in the resulting data frame. The syntax for this deliberately echoes how you would access the same variable in a modelling language like Stan or JAGS:
...
We can again simplify the above syntax, for two reasons: (1) `gather_samples` automatically groups its result by all indices you pass to it, and (2) As before, `mean_qi` will automatically select only non-grouping / non-special columns if you pass it an empty list; here that's just `group_mean`. So we can write:
