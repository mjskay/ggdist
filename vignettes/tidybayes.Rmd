...
```{stan, output.var = "ABC_stan", results = "hide", cache = TRUE}
data {
  int<lower=1> n;
...
Plotting medians and intervals is straightforward using the `pointinterval` geom (or its horizontal version, `pointintervalh`), which are simlar to `ggplot2::geom_pointrange` but with sensible defaults for multiple intervals (functionality we will use later):
...
The `stat_eye` and `stat_eyeh` geoms provide a shortcut to generating eye plots with some sensible defaults:
...
![The slabinterval family of geoms and stats](slabinterval_family.png)
...
Within the slabinterval family of geoms in tidybayes is the `heap` and `heapinterval` family, which automatically determine appropriate bin sizes for dotplots and can calculate quantiles from samples to construct quantile dotplots. `stat_heaph()` is the horizontal variant designed for use on samples:

```{r}
m %>%
  spread_draws(condition_mean[condition]) %>%
  ggplot(aes(x = condition_mean, y = fct_rev(condition))) +
  stat_heaph(quantiles = 100)
```
