...
### Compatibility with `lsmeans`

The `lsmeans` package provides a convenient syntax for generating marginal estimates from a model, including numerous types of contrasts. It also supports some Bayesian modeling packages, like `MCMCglmm` and `rstanarm`. However, it does not provide samples in a tidy format. The `gather_lsmeans_samples` function converts output from `lsmeans` into a tidy format, keeping the `lsmeans` reference grid and adding an `estimate` column with long-format samples.
...
We can use `lsmeans::lsmeans` to get conditional means with uncertainty:
...
Or `lsmeans::lsmeans` with `lsmeans::contrast` to do all pairwise comparisons:
...
See `?pairwise.lsmc` for a list of the numerous contrast types supported by `lsmeans`.
