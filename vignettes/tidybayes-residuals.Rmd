...
Then these residuals will be uniformly random (see [Dunn and Smyth 1996](http://doi.org/10.2307/1390802)), and we can similarly define a **randomized quantile residual** by passing the probability residuals through the Normal inverse CDF:

$$
z^{\textrm{resid}}_i = F_{\textrm{Normal}}^{-1}(p^{\textrm{resid}}_i)
$$

We in code, that looks like this:
...
```{r}
m_bin = stan_glm(y ~ 1, data = bin_df, family = binomial)
```
...
Randomized quantile residuals help us see that the fit is reasonable:

```{r}
p = bin_df %>%
  add_predicted_draws(m_bin) %>%
  make_probability_residuals(.prediction, y, n = 50) %>%
  ggplot(aes(sample = .z_residual)) +
  geom_qq() +
  geom_abline() +
  transition_manual(.residual_draw)
...
Finally, we'll round this out with a Poisson regression. Here's some count data:
...
count_df %>%
  ggplot(aes(x = y)) +
  geom_dotplot(binwidth = 1, dotsize = 0.75)
```

And a model:

```{r}
m_count = brm(y ~ 1, data = count_df, family = poisson)
```
...
Again, hard to interpret. Let's try randomized quantile residuals instead:

```{r}
p = count_df %>%
  add_predicted_draws(m_count) %>%
  make_probability_residuals(.prediction, y, n = 50) %>%
  ggplot(aes(sample = .z_residual)) +
  geom_qq() +
  geom_abline() +
