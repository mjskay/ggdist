...
The `stat_dist_slabinterval()` family of functions supports these objects.
...
```{r lineribbon}
lighten2 = Vectorize(function(color, amount) {
  rgb(colorRamp(c(color, "white"), space = "Lab")(amount), max = 255)
})
...
groups is to manually adjust the lightness of each ribbon using `ggplot2::stage()`,
which allows you to transform color values after they have been computed. Here
we'll combine it with `colorspace::lighten()` to lighten the fill color of
each band according to its width:
...
mtcars %>%
  ...
  stat_dist_lineribbon(aes(
    ...
    fill = stage(start = ordered(cyl), after_scale = colorspace::lighten(fill, .width * .9, space = "combined"))
  ), show.legend = FALSE) +
  geom_point(aes(y = mpg), data = mtcars) +
