...
So let's try half-eye plots instead. The basic idea is that we need to get the three parameters for the sampling distribution of each parameter and then use `stat_dist_halfeyeh()` to plot them. The sampling distribution for a parameter is then a scaled-and-shifted t distribution, with:

- degrees of freedom equal to `df.residual(m_ABC)`
- location equal to the estimate of a parameter
- scales equal to the standard error of a parameter
...
m_ABC %>%
  tidy() %>%
  ggplot(aes(y = term)) +
  stat_dist_halfeyeh(aes(dist = "student_t", arg1 = df.residual(m_ABC), arg2 = estimate, arg3 = std.error))
```

If we would rather see uncertainty in conditional means, we can instead use `modelr::data_grid()` along with `broom::augment()` (similar to how we use `modelr::data_grid()` along with `tidybayes::add_fitted_draws()` for the Bayesian case):
...
ABC %>%
  ...
  stat_dist_halfeyeh(aes(dist = "student_t", arg1 = df.residual(m), arg2 = .fitted, arg3 = .se.fit), scale = .5) +
...
ABC %>%
  ...
  augment(m, newdata = .) %>%
  ggplot(aes(y = condition)) +
  stat_dist_gradientintervalh(aes(dist = "student_t", arg1 = df.residual(m), arg2 = .fitted, arg3 = .se.fit), scale = .5)
```
