...
m %>%
  ...
  head(10)
```
*(10 rows of `r nrow(spread_draws(m, r_condition[condition,term]))`)*
...
m %>%
  ...
  head(10)
```
*(10 rows of `r nrow(spread_draws(m, sigma))`)*
...
m %>% 
  ...
  head(10)
```
*(10 rows of `r nrow(spread_draws(m, b_Intercept, r_condition[condition,]))`)*
...
ABC %>%
  ...
  head(10)
```
*(10 rows of `r nrow(ABC %>% data_grid(condition) %>% add_fitted_draws(m))`)*
...
fit_plot = mtcars %>%
  ...
  add_fitted_draws(m_cyl, value = "p(cyl)") %>%
  ...
  ggplot(aes(x = mpg, y = `p(cyl)`, color = cyl)) +
...
$$
\textrm{E}[\textrm{cyl}|\textrm{mpg}=m] = \sum_{cyl \in \{4,6,8\}} cyl\cdot p(cyl|\textrm{mpg}=m)
$$
We can use the above formula to derive a posterior distribution for $\textrm{E}[\textrm{cyl}|\textrm{mpg}=m]$ from the model. The model gives us a posterior distribution for $p(cyl|\textrm{mpg}=m)$: it is the response-scale linear predictor ("fit") for `cyl` (or `.category`) = $cyl$ when `mpg` = $m$. We can group with `.draw` and then use `summarise` to calculate the expected value:

```{r}
data_plot_with_mean = mtcars %>%
  data_grid(mpg = seq_range(mpg, n = 101)) %>%
  add_fitted_draws(m_cyl, value = "p(cyl)", n = 100) %>%
  ...
  summarise(cyl = sum(cyl * `p(cyl)`)) %>%
  ggplot(aes(x = mpg, y = cyl)) +
  geom_line(aes(group = .draw), alpha = 5/100) +
  geom_point(aes(color = ordered(cyl)), data = mtcars)
