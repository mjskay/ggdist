...
```{r}
m_cyl = brm(cyl ~ mpg, data = mtcars_clean, family = cumulative)
```
...
mtcars_clean %>%
  select(mpg) %>%
  add_predicted_draws(m_cyl, prediction = "cyl", seed = 12345) %>%
  ggplot(aes(x = mpg, y = cyl)) +
  ...
  geom_label_repel(
    ...
    label = "posterior predictions", xlim = c(26, NA), ylim = c(NA, 2.8), point.padding = 1,
    label.size = NA, color = "gray50", segment.color = "gray75"
  ) +
  geom_label_repel(
    data = mtcars_clean %>% filter(cyl == "6") %>% filter(mpg == max(mpg)) %>% dplyr::slice(1),
    label = "observed data", xlim = c(26, NA), ylim = c(2.2, NA), point.padding = 1,
    label.size = NA, segment.color = "gray35"
  ) 
```

This looks pretty good. Let's check using another typical posterior predictive checking plot: many simulated distributions of the response (`cyl`) against the observed distribution of the response:
...
mtcars_clean %>%
  ...
  ggplot(aes(x = cyl)) +
  stat_density(aes(group = .draw), geom = "line", position = "identity", color = "gray50", alpha = .1) +
  stat_density(aes(group = NA), geom = "line", data = mtcars_clean, color = "red", size = 1) +
  geom_label(data = data.frame(cyl = "4"), y = .32, label = "posterior\npredictions",
    hjust = 1, color = "gray50", lineheight = 1, label.size = NA) +
  geom_label(data = data.frame(cyl = "8"), y = .45, label = "observed data",
