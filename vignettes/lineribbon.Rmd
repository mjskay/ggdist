...
a continous fill scale (e.g. `scale_fill_distiller()`) instead of a discrete one
(e.g. `scale_fill_brewer()`):

```{r stat_lineribbon_gradient, fig.width = med_width, fig.height = med_height}
df %>%
  ggplot(aes(x = x, y = y)) +
...
```{r stat_lineribbon_gradient_ramp, fig.width = med_width, fig.height = med_height}
df %>%
  ggplot(aes(x = x, y = y)) +
...
```{r stat_lineribbon_2groups_brewer, fig.width = med_width, fig.height = med_height}
df_2groups %>%
...
```{r stat_lineribbon_2groups_alpha, fig.width = med_width, fig.height = med_height}
df_2groups %>%
...
```{r stat_lineribbon_2groups_ramp, fig.width = med_width, fig.height = med_height}
df_2groups %>%
  ggplot(aes(x = x, y = y, fill = g)) +
...
```{r mtcars_spaghetti, fig.width = med_width, fig.height = med_height}
mtcars_boot %>%
  filter(.draw < 400) %>%
...
```{r mtcars_point_interval, fig.width = med_width, fig.height = med_height}
mtcars_boot %>%
  ggplot(aes(x = mpg, y = hp)) +
...
```{r mtcars_curve_interval, fig.width = med_width, fig.height = med_height}
mtcars_boot %>%
  group_by(mpg) %>%
...
```{r mtcars_curve_interval_bd, fig.width = med_width, fig.height = med_height, eval = requireNamespace("fda", quietly = TRUE)}
mtcars_boot %>%
  group_by(mpg) %>%
